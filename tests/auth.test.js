...
describe('Auth Routes', () => {
  describe('POST /api/auth/register', () => {
    test('Doit créer un nouvel utilisateur avec des données valides', async () => {
      ...
      const res = await request(app).post('/api/auth/register').send(testData);
  ...
  describe('POST /api/auth/forgot-password', () => {
    test('Doit envoyer un email de réinitialisation pour un email valide', async () => {
      ...
      expect(res.body.message).toMatch(
        /Si cet email existe dans notre base, vous recevrez un lien de réinitialisation/i
      );
    });

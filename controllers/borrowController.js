  ...
  try {
    ...
    if (mediaType) {
      borrows = borrows.filter(
        borrow => borrow.media && borrow.media.type === mediaType
      );
    }
    ...
    if (search) {
      ...
      borrows = borrows.filter(borrow => {
        if (borrow.media) {
          ...
          return (
            mediaTitle.includes(searchTerm) || mediaAuthor.includes(searchTerm)
          );
        }
        return false;
      });
    ...
    res
      .status(500)
      .json({ message: 'Erreur lors de la récupération de vos emprunts' });
  }
};
    ...
    if (search || user) {
      ...
      borrows = borrows.filter(borrow => {
        ...
        if (borrow.media) {
          ...
          const mediaAuthor = borrow.media.author?.toLowerCase() || '';
          if (
            mediaTitle.includes(searchTerm) ||
            mediaAuthor.includes(searchTerm)
          ) {
            return true;
          }
        }

...
export const requestPasswordReset = async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      logger.info(
        `Demande de réinitialisation de mot de passe pour un email inexistant: ${email}`
      );
    ...
    if (!user.actif) {
      logger.warn(
        `Tentative de réinitialisation de mot de passe pour un compte désactivé: ${email}`
      );
    ...
    if (process.env.NODE_ENV !== 'test') {
      try {
        ...
        logger.info(
          `Email de réinitialisation de mot de passe envoyé à ${email}`
        );
      } catch (emailError) {
        logger.error(
          `Erreur lors de l'envoi de l'email de réinitialisation: ${emailError.message}`
        );
...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!token || !email || !newPassword) {
      return res
        .status(400)
        .json({ message: 'Token, email et nouveau mot de passe requis' });
    }
    ...
    if (!resetToken) {
      return res
        .status(400)
        .json({ message: 'Token de réinitialisation invalide' });
    }

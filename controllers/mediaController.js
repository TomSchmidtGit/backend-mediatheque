  ...
  try {
    ...
    if (!title || !type || !author || !year || !req.file) {
      return res.status(400).json({
        message:
          "Tous les champs obligatoires doivent être fournis, y compris l'image",
      });
    }
...
export const addReview = async (req, res) => {
  try {
    ...
    const existingReview = media.reviews.find(
...
export const updateReview = async (req, res) => {
  try {
    ...
    if (!review) {
      return res
        .status(404)
        .json({ message: "Vous n'avez pas encore noté ce média" });
    }
